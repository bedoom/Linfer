...
sys.path.append('/home/hzq/projects/fast-reid')
...
from fastreid.evaluation.rank_cylib import compile_helper
...
if __name__ == "__main__":   
    ...
    cfg.TEST.ROC.ENABLED = True
    cfg.TEST.METRIC = "cosine"
    with open('/home/hzq/projects/Linfer/workspace/query_reid.json', 'r') as f:
        data = json.load(f)
    query_features = sum(len(item['features']) for item in data)

    evaluator = ReidEvaluator(cfg, query_features)

    for item in data:
        features = torch.tensor(item['features'], dtype=torch.float32)
        features_len = len(features)
        pids = torch.tensor([int(item['reid'])]*features_len, dtype=torch.int)
        camids = torch.tensor([0]*features_len, dtype=torch.int)

        inputs = {'targets': pids, 'camids': camids}
        outputs = features
        evaluator.process(inputs, outputs)

    with open('/home/hzq/projects/Linfer/workspace/gallery_reid.json', 'r') as f:
        data = json.load(f)
    for item in data:
        features = torch.tensor(item['features'], dtype=torch.float32)
        features_len = len(features)
        pids = torch.tensor([int(item['reid'])]*features_len, dtype=torch.int)
        camids = torch.tensor([1]*features_len, dtype=torch.int)
